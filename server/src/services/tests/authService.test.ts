...
const mockUserRepository = {
  ...
  getByUserName: vi.fn(),
  getById: vi.fn(),
  getByEmailWithRoleName: vi.fn(),
  getByIdWithRoleName: vi.fn(),
  create: vi.fn(),
...
const testUserPublic: UserPublic = {
  ...
  name: fakeTestUserWithHash.name,
}

const testAuthUserWithRoleName: AuthUserWithRoleName = {
  id: fakeTestUserWithHash.id,
  name: fakeTestUserWithHash.name,
  roleName: fakeTestUserWithHash.roleName,
}
...
describe('signup', () => {
  ...
  it('should throw error if email already exists', async () => {
    ...
    // Act & Assert
    await expect(
      authService.signup('test@example.com', 'test-user', 'password123', 2)
    ).rejects.toThrow(TRPCError)

    const error = await authService
      .signup('test@example.com', 'test-user', 'password123', 2)
      .catch((err) => err)
...
describe('login', () => {
  ...
  it('should successfully login with valid credentials', async () => {
    ...
    mockUserRepository.getByEmailWithRoleName.mockResolvedValue(
      fakeTestUserWithHash
    )
    ...
    expect(result).toEqual({
      ...
      user: testAuthUserWithRoleName,
    })
    expect(mockUserRepository.getByEmailWithRoleName).toHaveBeenCalledWith(
      'test@example.com'
    )
  ...
  it('should throw error for non-existent user', async () => {
    // Arrange
    mockUserRepository.getByEmailWithRoleName.mockResolvedValue(null)
...
describe('refreshTokens', () => {
  ...
  it('should successfully refresh tokens', async () => {
    ...
    mockUserRepository.getByIdWithRoleName.mockResolvedValue(
      fakeTestUserWithHash
    )
    ...
    expect(mockUserRepository.getByIdWithRoleName).toHaveBeenCalledWith(
  ...
  it('should throw error if user no longer exists', async () => {
    // Arrange
    mockUserRepository.getByIdWithRoleName.mockResolvedValue(null)
  ...
  it('should handle incorrect token type validation (BUG TEST)', async () => {
    // Arrange
    mockUserRepository.getByIdWithRoleName.mockResolvedValue(fakeTestUserWithHash)
    ;(tokenPayloadModule.parseTokenPayload as Mock).mockReturnValue({
      user: testUserPublic,
